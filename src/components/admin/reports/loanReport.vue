  ...
  <div class="container">
        ...
        <div class="row" style="margin-top:-30px">
          <div class="col">
            ...
            <div class="row mt-5">
              ...
              <div class="col-md-5">
                <div class="input-group">
                  <input
                    type="search"
                    placeholder="search by name, status, date, month or year"
                    v-model="name"
                    class="form-control"
                    name
                    id
              ...
              <div class="col-md-5">
                <div class="input-group">
                  ...
                  <datepicker
                    v-model="fromDate"
                    id="date1"
                    style="border:1px solid blu; border-radius:3px;"
                    placeholder="Select Date"
                  ></datepicker>
                  <div class="input-group-append">
                    <span class="input-group-text" style="height:30px" id="inputGroupPrepend">
            ...
            <div class="table-responsive" ref="html" id="html">
              ...
              >Loan Report from {{customFormatter(fromDate)}} to {{customFormatter(toDate)}}</span>
                ...
                <tbody v-if="downloadLoanRequests">
                  <tr v-for="(item, index) in downloadLoanRequests" :key="'a' + index">
                    <th
                      scope="row"
                      class="text-success"
                    >{{item.userDetails.firstName}} {{item.userDetails.lastName}}</th>
    ...
    <div class="row mt-2 d-flex justify-content-around">
      ...
      <div
        ...
        v-show="pageCount>0"
        @click="exportTableToExcel('html')"
      >
        Download excel
        <i class="fa fa-download"></i>
      </div>
      <div class="btn btn-primary float-right" v-show="pageCount>0" @click="exportToWord('html')">
        Download Word
        <i class="fa fa-download"></i>
      </div>
      <div class="btn btn-primary float-right" v-show="pageCount>0" @click="exportToPDF">
        Download pdf
        <i class="fa fa-download"></i>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    formatNumber(str) {
      str = str.toString();
      return (
        str.substring(0, 4) + " " + str.substring(4, 7) + " " + str.substring(7)
      );
    },
    moment(x) {
    ...
    sortedDataByDate() {
      ...
      let filtered = data.filter(item => {
        let filteredDate =
          this.moment(item.createdAt) >= this.moment(this.fromDate) &&
          this.moment(item.createdAt) <= this.moment(this.toDate);
        return filteredDate;
      });
  ...
  computed: {
    ...
    forward() {
      if (this.pageNumber == this.pageCount - 1 || this.pageCount == 0)
        return false;
      return true;
    },
    pageCount() {
    ...
    paginatedData() {
      ...
      let order = this.sortedDataByDate().sort(
    ...
    filteredLoanRequests() {
      if (this.name == "") {
        ...
        return result.filter(value => {
          ...
          let fullNameReverse =
            value.userDetails.lastName + " " + value.userDetails.firstName;
          ...
          return (
            value.userDetails.firstName.indexOf(this.name) == 0 ||
