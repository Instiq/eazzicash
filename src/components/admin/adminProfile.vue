<template>
  <div class>
    <section class="mb-5 d-flex position-fixed header-nav" style>
      <p
        v-if="['adminDashboard'].includes($route.name)"
        class="text-white profileItems"
        style="font-size:19px; margin-top:24px;  font-weight:bold;   margin-left:25%"
      >Dashboard</p>
      <p
        v-if="['approvals'].includes($route.name)"
        class="text-white profileItems"
        style="font-size:19px; margin-top:24px;  font-weight:bold; margin-left:25%"
      >Approvals</p>
      <p
        v-if="['userManagement'].includes($route.name)"
        class="text-white profileItems"
        style="font-size:19px; margin-top:24px;  font-weight:bold; margin-left:25%"
      >User Management</p>
      <p
        v-if="['userDetails'].includes($route.name)"
        class="text-white profileItems"
        style="font-size:19px; margin-top:24px;  font-weight:bold; margin-left:25%"
      >User Management</p>
      ...
      <div
        icon="envelope"
        right
        class="float-right py-4 d-flex"
        style="border:2px solid blu;flex-direction:row"
      >
        ...
        <a @click="userSignOut" class="mr-5 text-white">
    ...
    <section id="mySidenav" class="sidenav" style="z-index:2">
      ...
      <!-- profile items -->
      <router-link to="/adminProfile/adminDashboard">
        <img src="./adminIcons/Home.svg" height="15vh" style alt />
        <span class="sidenav-items" style="margin-left:16px">Dashboard</span>
      </router-link>
      <router-link to="/adminProfile/userManagement">
        <i class="fas fa-user"></i>
        <span class="sidenav-items" style="margin-left:18px">User Management</span>
      </router-link>
      <router-link to="/adminProfile/approvals">
        <i class="fas fa-check"></i>
        <span class="sidenav-items" style="margin-left:15px">Approvals</span>
      </router-link>
      <router-link to="/adminProfile/reports">
        <img src="./adminIcons/Investments.svg" height="14vh" style alt />
        <span class="sidenav-items" style="margin-left:9px">Reports</span>
      </router-link>
    </section>
